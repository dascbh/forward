...
- **the kernel** — this repository: `${CLAUDE_PLUGIN_ROOT}` when installed
  as a plugin, otherwise a clone. It must live OUTSIDE the project tree —
  a kernel inside the project would be read by the stack detection in
  step 1 and describe the wrong repository.
